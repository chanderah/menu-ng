...
export class OrderDialogComponent extends SharedUtil implements OnInit {
  ...
  onSelectOptionValue(optionIndex: number, optionValueIndex: number) {
    this.optionValues(optionIndex).controls.forEach((ctrl, i) => {
      if (i !== optionValueIndex) ctrl.get('selected').setValue(false);
    });
  }
  ...
  onClickWa() {
    let url = 'https://api.whatsapp.com/send?phone=6287798992777&text=Halo,%20saya%20ingin%20menanyakan%20tentang%20';
